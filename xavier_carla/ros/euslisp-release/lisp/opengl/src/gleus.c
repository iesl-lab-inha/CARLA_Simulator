...
static int register_gleus()
  { add_module_initializer("___gleus", ___gleus);}

static pointer F200probe_extension();

/*probe-extension*/
static pointer F200probe_extension(ctx,n,argv,env)
...
IF203:
	...
	ctx->vsp=local+0;
	compfun(ctx,fqv[12],module,F200probe_extension,fqv[13]);
	local[0]= NIL;
	ctx->vsp=local; return(local[0]);}
static void init_ftab()
{  register int i;
