...
void TunerBody::saveResult(std::string fileName)
{
  ...
  /* write data to file */
  {
    CV::WriteStructContext st_red(cvfs, "RED", cv::FileNode::MAP);
    // cv::internal::WriteStructContext st_red(cvfs, "RED", CV_NODE_MAP);
   	{
      CV::WriteStructContext st_hue(cvfs, "Hue", cv::FileNode::MAP);
    ...
    {
      CV::WriteStructContext st_hue(cvfs, "Saturation", cv::FileNode::MAP);
      cv::write(cvfs, "center", Red_set.sat.center);
      cv::write(cvfs, "range", Red_set.sat.range);
    }

    {
	  CV::WriteStructContext st_hue(cvfs, "Value", cv::FileNode::MAP);
  ...
  {
	CV::WriteStructContext st_yellow(cvfs, "YELLOW", cv::FileNode::MAP);  
  ...
  {
    CV::WriteStructContext st_green(cvfs, "GREEN", cv::FileNode::MAP);
    {
       CV::WriteStructContext st_hue(cvfs, "Hue", cv::FileNode::MAP);
...
void TunerBody::openSetting(std::string fileName)
{
  ...
  /* read data from file */
   cv::FileNode topNode = cvfs.root(); // (cvfs.fs, NULL);
  {
